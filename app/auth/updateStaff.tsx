// app/auth/updateStaff.tsx

import { getStaffProfileByEmail, updateStaffPassword } from "@/lib/actions/staff.actions";
...
function UpdateStaffPassword() {
  ...
  const handleVerifyEmail = async () => {
    ...
    setLoading(true);
    try {
      const profile = await getStaffProfileByEmail(email.trim());
      if (!profile) {
  ...
  const handleChangePassword = async () => {
    ...
    setLoading(true);
    try {
      const result = await updateStaffPassword(email.trim(), currentPassword, newPassword);
      if (result.success) {
        Alert.alert(
          "Success",
          ...
          [
            {
              text: "OK",
              onPress: () => router.replace("/auth/staff"),
            },
          ],
          { cancelable: false }
        );
    ...
    <View className="relative">
      <View className="flex-row items-center bg-white/90 rounded-xl px-4 py-3">
        ...
        <TouchableOpacity onPress={() => setVisible(!visible)}>
          {visible ? <EyeOff size={18} color="#7c3aed" /> : <Eye size={18} color="#7c3aed" />}
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-transparent" edges={['bottom', 'left', 'right']}>
      <LinearGradient colors={["#7c3aed", "#a78bfa"]} style={{ flex: 1 }}>
        <KeyboardAvoidingView
          className="flex-1"
          ...
          >
            <View className="flex-1 p-6">
              ...
              <View className="items-center mt-10 mb-10">
                ...
                <Text className="text-base text-white/90 text-center mt-2">Update your staff account password</Text>
                {greetName ? (
                  <Text className="text-lg text-white mt-3">Hey {greetName} 👋</Text>
                ) : null}
              </View>
                ...
                {!emailVerified ? (
                  ...
                  >
                    {loading ? (
                      ...
                      <Text className="text-purple-600 text-base font-semibold">Continue</Text>
                    )}
                  </TouchableOpacity>
                ) : null}
                  ...
                  <>
                    ...
                    >
                      {loading ? (
                        ...
                        <Text className="text-purple-600 text-base font-semibold">Update Password</Text>
                      )}
                    </TouchableOpacity>
                  </>
                ) : null}
